...
box_length = 40 
box_width = 20 
s = (cq.Workplane("XY").box(box_length, box_width,2.5)
     
     .faces(">Z").workplane().pushPoints([(box_length-25,0)]).hole(hole_diameter)
     .faces(">Z").workplane().pushPoints([(-15,5),(-15,-5)]).hole(4) 
      
                  
    )

part = (
        cq.Workplane("XY").box(5,20,5).translate((-15,0,-2.6))
        .faces(">Z").workplane().pushPoints([(-15,5),(-15,-5)]).hole(4)
  
        
)
dovel = (
        cq.Workplane("XY").box(7.5,7.5,7.5).translate((box_length-25,0,0))
        .faces(">Z").workplane().pushPoints([(box_length-25,0)]).hole(hole_diameter)
) 
...
motor_board = (
    cq.Workplane("XY")    
    .rect(24+24+16+8+4, board_y_outer+16+6)
    .extrude(plate_t)
)
"""
battery_board_16 = (
...
b2= battery_board_16.translate((16+16+2,0,0))
motor_board = motor_board.cut(b2)
