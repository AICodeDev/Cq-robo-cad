...
def create_motor_plate(length=24, width=16, thickness=4, hole_diameter=3.2):
    """
    Simple motor mounting plate with 4 holes.
    
    Parameters:
    -----------
    length : float - length of plate (vertical on Z axis)
    width : float - width of plate (along Y axis)
    thickness : float - thickness of plate
    hole_diameter : float - diameter of mounting holes
    """
    # Create rect plate
    plate = (
        cq.Workplane("YZ")  # Plate on YZ plane (vertical)
        .box(width, length, thickness)
    )
    
    # Add 4 mounting holes for motor
    hole_spacing_x = width / 3
    hole_spacing_y = length / 3
    ...
    plate = (
        ...
        .workplane()
        .pushPoints([
            (-hole_spacing_x, -hole_spacing_y),  # Bottom-left
            (hole_spacing_x, -hole_spacing_y),   # Bottom-right
            (-hole_spacing_x, hole_spacing_y),   # Top-left
            (hole_spacing_x, hole_spacing_y)     # Top-right
        ])
...
motor_plate_left = create_motor_plate(
    ...
    width=16,
    thickness=4,
    hole_diameter=3.2  # M3 clearance holes for motor
)
...
# Position LEFT motor plate
motor_left_positioned = motor_plate_left.translate((
    0,                    # Center in X
    -plate_y/2 + 2,      # Left side, 2mm from edge
    12                   # Height above base
))

# Position RIGHT motor plate
motor_right_positioned = motor_plate_right.translate((
    0,                    # Center in X
    plate_y/2 - 2,       # Right side, 2mm from edge
    12                   # Height above base
))
...
# final = plateHole2.union(holder_positioned).union(motor_left_positioned).union(motor_right_positioned)
