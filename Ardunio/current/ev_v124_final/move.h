...
void slowStart(){
  ...
  int maxSpeed=700;
  int incr =5 ;
  unsigned int wait_ums = 300; // microselc
  int c=0;
  ...
  for (int i = minSpeed; i <= maxSpeed ; i = i + incr){
    ...
    #if DEBUG
      if (c%15==0){
            Serial.println("slowStart Phase -1 ");
            Serial.println(c);
...
void moveEvPulseNewLogic(long target_pulse, int dir)
{
   // delay(2000);
    ...
    Serial.println(target_pulse);
    PrintMove();
    Serial.println("NextMove Serial.printlnsetup done moveEvPulse moveEvPulse");
    ...
    long DO_NOT_CHECK_BEFORE_PENDING = 2000 ;
    ...
    #if DEBUG
            Serial.println("Slow Phase 0 Continue");
    ...
    delay(5);  
    #if DEBUG
            Serial.println("Slow Phase 0 Continue-befoe max speed");
    ...
    UpdateMoveStateEv();
    #if DEBUG
            Serial.println("Phase 0 Continue=part2 after max ");
    
      PrintMove();
    
    #endif
     long rem_pulse=curr.target_pulse- (curr.current_pulse.m1 );

     float rem_pulse_factor = 2.3 ;// got high friction
     DO_NOT_CHECK_BEFORE_PENDING = 300 ;
    #ifdef BIG_WHEEL
        rem_pulse_factor = 1.1 ;// got high friction
        DO_NOT_CHECK_BEFORE_PENDING = 2000 ;
    #endif;
    ...
    while (curr.current_pulse.m1 < first_target )
    {
      ...
      rem_pulse= (curr.target_pulse- (curr.current_pulse.m1 ) )  * rem_pulse_factor ;

      if ( ( (first_target -curr.current_pulse.m1) > DO_NOT_CHECK_BEFORE_PENDING)  
          && (rem_pulse > curr.short_pps.m1)   
      ...
      //1 second slow down time;
      //if (rem_pulse<curr.short_pps.m1){
      //  break;
     // }
...
void moveEvPulse(long target_pulse, int dir)
{
 ...
 PrintMove();
    moveEvPulseNewLogic(target_pulse,dir);
     delay(1000);
     PrintMove();
     StopMotors();
     delay(1000);
